		...
		for await (const saved_channel of youtube_channels) {
			...
			if (
				...
				for (const channel of response) {
					...
					await cacheSystem.set(channel.channel_id, JSON.stringify(channel)); // save it to the cache!
					if (dirExists == false) {
									...
									QuickMakeEmbed(
										{
											...
											description: `${getUser.subscriberIDs.length.toLocaleString(
												...
												getUser.subscriberIDs.length == 1 ? ' was' : 's were'
											} notified of this`,
										},
										undefined,
										djs_client,
										...
										.addFields({
											...
											value: `${
												Math.floor(
													getUser.currentUpdate.sub_rate * (86400 * 1000) || 0
												...
												Math.floor(((getUser.currentUpdate?.sub_rate ?? 0) -
													(getUser.lastUpdate?.sub_rate ?? 0)) *
													(86400 * 1000) || 0).toLocaleString('en-US') ?? 'None'
											}) _ _`,
											inline: true,
										})
										.addFields({
